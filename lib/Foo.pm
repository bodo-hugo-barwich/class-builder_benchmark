...
package Foo {
    ...
    my $TITLE_MAPPING = {
        ...
        'Foo::Mojo'              => sprintf( '`Mojo::Base@%s`',                       $Mojolicious::VERSION ),
    ...
    sub run_benchmark($create_main_logic_coderef) {
        ...
        my ($x) =
          map { $header->[$_] =~ /class feature/ ? ($_) : () } 0 .. $#$header;
    ...
    sub run_memory_consumption_benchmark($create_main_logic_coderef) {
        ...
        my ($size_of_feature_class) =
          map { $_ =~ /class feature/ ? ( $result{$_} ) : () } keys %result;
